 ...
 */
public class _299_BullsAndCows implements Easy {
  ...
  public static class Solution1 implements Solution {
    ...
    public String getHint(String secret, String guess) {
      ...
      for(int d = 0; d < secret.length(); d++) {
        if(secret.charAt(d) == guess.charAt(d)) {
          bulls++;
        } else {
          secretCounts[secret.charAt(d) - '0']++;
          guessCounts[guess.charAt(d) - '0']++;
        }
      }
      int cows = 0;
      for(int n = 0; n < 10; n++) {
        cows += Math.min(secretCounts[n], guessCounts[n]);
      }
